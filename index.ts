...
// Get the NGINX Ingress Controller service to retrieve the load balancer hostname
const ingressService = k8s.core.v1.Service.get("ingress-nginx-controller", 
    pulumi.interpolate`ingress-nginx/ingress-nginx-controller`,
    { provider: k8sProvider, dependsOn: [nginxIngress] }
);

const loadBalancerHostname = ingressService.status.apply(status => {
    if (status && status.loadBalancer && status.loadBalancer.ingress && status.loadBalancer.ingress.length > 0) {
        return status.loadBalancer.ingress[0].hostname || status.loadBalancer.ingress[0].ip || "pending";
    }
    return "pending";
});
...
export const ingressLoadBalancer = loadBalancerHostname;
